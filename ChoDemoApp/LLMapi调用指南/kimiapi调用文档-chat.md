...
使用拼http请求调用 推荐cronet+：OkHttp（底层）+ Retrofit（接口定义）      
序列化：kotlinx.serialization 或 Moshi                                                                                                                         
•并发：Kotlin Coroutines（suspend/Flow）                                                                                                                        
•流式输出：如果是 SSE，直接用 OkHttp 读 ResponseBody 流逐行解析；或用支持 SSE 的封装（但你仍要能兜底到原生流式读取）       

eg：curl https://api.moonshot.cn/v1/chat/completions \
...
# 其中 $MOONSHOT_API_KEY 需要替换为.env的 API Key。
